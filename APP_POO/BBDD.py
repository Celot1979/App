...
def conexionBBDD():
    miConexion = sqlite3.connect("Datos_POO.db")
    miCursor= miConexion.cursor()
    try:
        miCursor.execute("""CREATE TABLE formulario_POO(ID INTEGER PRIMARY KEY AUTOINCREMENT,
...
def eliminar_Tabla():
    miConexion = sqlite3.connect("Datos_POO.db")
    miCursor= miConexion.cursor()
    miCursor.execute("DROP TABLE formulario_POO")
...
def crear(nombre,apellidos,contraseña,direccion,l6):
    ...
    try:
        ...
        miCursor.execute("INSERT INTO formulario_POO VALUES(NULL,?,?,?,?,?)",(datos_for))
...
def leer(id,nombre,apellidos,contraseña,direccion,l6):
    miConexion = sqlite3.connect("Datos_POO.db")
    miCursor = miConexion.cursor()
    miCursor.execute("SELECT * FROM formulario_POO WHERE ID =" + id.get())
...
def actualizar(id,nombre,apellidos,contraseña,direccion,l6):
    miConexion = sqlite3.connect("Datos_POO.db")
    miCursor = miConexion.cursor()
    miCursor.execute("UPDATE formulario_POO SET NOMBRE='" + nombre.get() +
        "', APELLIDOS = '" + apellidos.get() +
        "', CONTRASEÑA =  '" + contraseña.get() +
        "', DIRECCION =  '" + direccion.get() +    
        "', COMENTARIO = '" + l6.get(1.0, "end-1c") +
        "' WHERE ID=" + id.get())
    miConexion.commit()
...
def borrar(id,nombre,apellidos,contraseña,direccion,l6):
        miConexion = sqlite3.connect("Datos_POO.db")
        miCursor = miConexion.cursor()
        miCursor.execute("DELETE FROM formulario_POO WHERE ID =" + id.get())
        miConexion.commit()
        id.set('')
        nombre.set('')
        apellidos.set('')
